...
/**
 * Downloads a mindmap as PDF or image (PNG/JPEG), using SVG if available.
 * @param {HTMLElement} element - The DOM node of the mindmap card (should contain React Flow SVG).
 * @param {string} title - The desired filename (without extension).
 * @param {string} format - 'pdf', 'png', or 'jpeg'.
 */
export const handleMindmapDownload = async (element, title = 'mindmap', format = 'pdf') => {
  if (!element) {
    alert('Mindmap element not found.');
    return;
  }
  const safeTitle = (title || 'mindmap').replace(/[\/\\?%*:|"<>]/g, '-');

  try {
    if (format === 'png') {
      const dataUrl = await toPng(element, { pixelRatio: 3, backgroundColor: '#fff' });
      const link = document.createElement('a');
      link.href = dataUrl;
      ...
      const dataUrl = await toJpeg(element, { quality: 1, pixelRatio: 3, backgroundColor: '#fff' });
      const link = document.createElement('a');
      link.href = dataUrl;
      link.download = `${safeTitle}.jpeg`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
    } else if (format === 'pdf') {
